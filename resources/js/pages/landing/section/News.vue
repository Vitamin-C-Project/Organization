...
<template>
    <section id="news" class="bg-amber-50/50 py-20">
        <div class="container mx-auto px-4">
            <h2 class="!mb-12 text-center text-3xl font-bold text-white">Latest News</h2>
            <div class="grid grid-cols-1 gap-8 md:grid-cols-3">
                <div v-for="article in articles" :key="article.id" class="news-card">
                    ...
                    <div class="p-6">
                        ...
                        <h3 class="!mb-2 text-xl font-semibold text-amber-900">{{ article.title }}</h3>
                        <p class="!mb-4 line-clamp-2 text-sm text-amber-700" v-html="article.content"></p>
                        <Link :href="`/news/${article.slug}`" class="group flex items-center space-x-2 text-amber-800 hover:text-amber-600">
